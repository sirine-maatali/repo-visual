...
def extract_test_data(test_execution_folder, test_cases_folder, defects_folder, file_name):
    ...
    for item in customfield_data:
        ...
        test_case_file_path = os.path.join(test_cases_folder, f"{test_key}.json")
        test_case_data = read_json_file(test_case_file_path) or {}

        feature = test_case_data.get("fields", {}).get("customfield_13601", "null")

        defects_file_path = os.path.join(defects_folder, f"{file_name}.json")
        defects_data = read_json_file(defects_file_path) or []
        ...
        output_data.append({
            ...
            "testKey": test_key,
            "testRunId": test_run_id,
            "feature": feature.strip("[]'"),  # Correction pour normaliser le champ feature
            "status": status,
            "defects": defects_info
        })
...
def save_output_by_feature(output_data, output_file):
    ...
    feature_grouped = defaultdict(list)
    for item in output_data:
        feature_grouped[item["feature"]].append(item)
...
if __name__ == "__main__":
    ...
    output_data, defect_to_testkey, defect_counts = extract_test_data("test execution", "test cases", "testCase Defects", file_name)
    save_output_to_json(output_data, "output.json")
    save_output_by_feature(output_data, "output2.json")
