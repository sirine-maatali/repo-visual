        ...
        stage('Exécuter le script Python') {
            steps {
                script {
                    ...
                    // Exécuter le script avec le paramètre FILE_NAME
                    bat "python app.py ${params.FILE_NAME} output.json"
                    ...
                    jsonData.each { item ->
                        ...
                        if (!featureStatusMap.containsKey(feature)) {
                            featureStatusMap[feature] = [:]
                        }
                        if (!featureStatusMap[feature].containsKey(status)) {
                            featureStatusMap[feature][status] = 0
                        }
                        featureStatusMap[feature][status]++
                    }

                    // Transformer les données pour Chart.js
                    def featureLabels = featureStatusMap.keySet() as List
                    def statusTypes = featureStatusMap.values().collectMany { it.keySet() }.unique()
                    def datasets = statusTypes.collect { status ->
                        def data = featureLabels.collect { feature -> featureStatusMap[feature].getOrDefault(status, 0) }
                        return [
                            label: status,
                            data: data,
                            backgroundColor: "rgba(${(Math.random() * 255).toInteger()}, ${(Math.random() * 255).toInteger()}, ${(Math.random() * 255).toInteger()}, 0.7)"
                        ]
                    }
                    ...
                    // Génération du fichier HTML avec Chart.js
                    def htmlContent = """
                        <html>
                        <head>
                            <title>Test Execution - ${params.FILE_NAME}</title>
                        ...
                        <body>
                            <div class="container">
                                <h1>Test Execution</h1>
                                <h2>Nom du fichier : ${params.FILE_NAME}</h2>
                                <h3>Histogramme des Features et Statuts</h3>
                                <canvas id="barChart"></canvas>
                            </div>
                            <script>
                                var ctx = document.getElementById('barChart').getContext('2d');
                                var chartData = ${jsonChartData.replace("'", "\"")}; // Convertir en format JS compatible
